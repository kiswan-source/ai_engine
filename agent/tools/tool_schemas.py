...
Canonical home as of Fase 14 (DCF v5 mandate — orchestrator agent tool
access): this used to live only at ``core/chat/tool_schemas.py``. Moved here,
next to ``agent/tools/registry.py`` (the one place every agent path already
gets its tools from), so ``agents/generic_agent.py``'s new EXECUTOR-capability
tool-calling sub-loop can reuse the exact same curated schema list without
``agents/`` importing from ``core/chat/`` — that direction would invert this
codebase's established dependency rule (``core/chat/`` depends on
``agents/``/``orchestrator/``, never the reverse; see
``agent/tools/workspace_reader.py``'s docstring for the same rule applied to
``api/``). ``core/chat/tool_schemas.py`` is now a thin re-export so every
existing ``from core.chat.tool_schemas import ...`` keeps working unchanged.
"""
...
# Fase 15 (bulk/large-file capability): the 6 text-bearing readers below all
# accept the same offset/length pagination pair — a document larger than one
# window sets has_more=true in its response; call again with a bigger offset
# to continue. Both optional; omitting them reproduces the pre-Fase-15
# behavior exactly (first 10000 chars).
_PAGINATION_PROPS = {"offset": _INT, "length": _INT}
_PAGINATION_HINT = (
    " Dokumen besar dipotong per halaman (default 10000 karakter) — kalau respons "
    "punya has_more=true, panggil lagi dengan offset lebih besar untuk lanjut membaca."
)

TOOL_SCHEMAS = [
    # ── Readers ──
    _fn("read_pdf", "Baca/ekstrak teks dari file PDF." + _PAGINATION_HINT,
        {"file_path": _STR, **_PAGINATION_PROPS}, ["file_path"]),
    _fn("read_docx", "Baca dokumen Word (.docx)." + _PAGINATION_HINT,
        {"file_path": _STR, **_PAGINATION_PROPS}, ["file_path"]),
    _fn("read_csv", "Baca file CSV (header + baris).",
        {"file_path": _STR}, ["file_path"]),
    _fn("read_txt", "Baca file teks/markdown." + _PAGINATION_HINT,
        {"file_path": _STR, **_PAGINATION_PROPS}, ["file_path"]),
    _fn("read_json", "Baca file JSON." + _PAGINATION_HINT,
        {"file_path": _STR, **_PAGINATION_PROPS}, ["file_path"]),
    ...
    _fn("read_shp", "Baca Shapefile (.shp atau .zip) → ringkas.",
        {"file_path": _STR}, ["file_path"]),
    _fn("read_xlsx", "Baca isi spreadsheet Excel (.xlsx), per-sheet." + _PAGINATION_HINT,
        {"file_path": _STR, **_PAGINATION_PROPS}, ["file_path"]),
    _fn("read_pptx", "Baca isi presentasi PowerPoint (.pptx), per-slide." + _PAGINATION_HINT,
        {"file_path": _STR, **_PAGINATION_PROPS}, ["file_path"]),
    _fn("read_many_files",
        "Baca BEBERAPA file sekaligus (maks 50) dalam satu panggilan, satu per satu tetap dipetakan ke pembaca yang "
        "sesuai berdasarkan ekstensi (pdf/txt/md/log/docx/csv/json/xlsx/pptx/gambar). Pakai ini alih-alih memanggil "
        "read_pdf/read_docx/dst berulang kali kalau perlu baca banyak file dalam satu tugas. Satu file gagal tidak "
        "menggagalkan seluruh batch — cek key 'error' per hasil. length_per_file (opsional, default 2000) membatasi "
        "isi teks per file supaya total hasil tidak terlalu besar untuk banyak file sekaligus.",
        {"file_paths": {"type": "array", "items": _STR}, "length_per_file": _INT}, ["file_paths"]),
    ...
    _fn("workspace_list_files", "Daftar semua file di Project Workspace yang terhubung ke sesi chat ini (folder lokal yang diregistrasikan). Panggil ini dulu sebelum membaca file dari Workspace.",
        {}, []),
    _fn("workspace_read_file", "Baca satu file dari Project Workspace. Dokumen (pdf/txt/docx/csv/json) dikembalikan sebagai teks. Gambar (jpg/png/dll) akan DITAMPILKAN ke kamu sebagai gambar sungguhan pada giliran berikutnya — deskripsikan isinya. File GIS (kml/geojson/shp) dikembalikan sebagai ringkasan luas/centroid. Pakai folder_id dan relative_path PERSIS dari hasil workspace_list_files." + _PAGINATION_HINT,
        {"folder_id": _STR, "relative_path": _STR, **_PAGINATION_PROPS}, ["folder_id", "relative_path"]),
    _fn("workspace_read_many_files",
        "Baca BEBERAPA file sekaligus (maks 50) dari Project Workspace dalam satu panggilan, alih-alih memanggil "
        "workspace_read_file berulang kali. Semua file harus berada di folder_id yang sama. Satu file gagal tidak "
        "menggagalkan seluruh batch. Pakai folder_id PERSIS dari hasil workspace_list_files.",
        {"folder_id": _STR, "relative_paths": {"type": "array", "items": _STR}, "length_per_file": _INT},
        ["folder_id", "relative_paths"]),
    _fn("workspace_write_file", "Buat atau edit satu file LANGSUNG di Project Workspace, bukan di folder laporan biasa. Pakai ini saat pengguna minta membuat/mengedit file di dalam Workspace/folder proyek mereka. File TEKS (txt/md/log/csv/json/html): mode='overwrite' (default, ganti seluruh isi) atau mode='append' (tambahkan ke akhir file). File DOCX/XLSX/PPTX/PDF SUNGGUHAN: kirim isi sebagai teks markdown di `content` (heading #/##, list -/*, **tebal**, tabel didukung), beri `title` (opsional, kalau tak diisi diambil dari nama file). DOCX/XLSX/PPTX hanya mendukung mode='overwrite' (ganti seluruh isi). PDF mendukung mode='overwrite' ATAU mode='append' (tambah halaman baru di akhir PDF yang sudah ada, TANPA mengubah halaman lama — `title` jadi judul section baru itu). DOCX juga mendukung mode='edit' (edit satu section SAJA tanpa mengubah bagian lain): isi `heading` dengan nama section yang mau diedit (harus cocok persis dengan teks heading yang sudah ada di file), `content` jadi isi BARU section itu (JANGAN ulangi heading-nya lagi di content) — kalau heading itu belum ada di file, otomatis ditambahkan di akhir dokumen. Pakai folder_id PERSIS dari hasil workspace_list_files.",
        {"folder_id": _STR, "relative_path": _STR, "content": _STR,
         "mode": {"type": "string", "enum": ["overwrite", "append", "edit"]},
         "title": _STR, "heading": _STR},
