...
- **Delete (file/folder) still doesn't exist** — deliberately deferred to a separate Slice 2, gated on a two-step confirmation token (Owner decision, Gate 1: delete is irreversible, classified HUMAN-ONLY by the DCF decision engine) mirroring the orchestrator's `HumanApprovalGate` shape, not a same-turn action. xlsx/pptx support and format-preserving in-place DOCX/PDF edit (today's `workspace_write_file` is still full-replace only for pdf/docx) are Slices 3/4 — new dependencies, higher design risk, not built yet.
